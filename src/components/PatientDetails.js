            ...
            <tbody>
              {appointments.map((appt, index) =>
                appt.visits
                  ...
                  .map((visit, vIndex) => (
                    <tr key={`${index}-${vIndex}`}>
                      <td>{visit.date ? new Date(visit.date).toLocaleDateString("en-IN", { dateStyle: "medium" }) : "N/A"}</td>
                      <td>{(visit.service || []).map((s) => (typeof s === "object" ? s.name : s)).join(", ") || "N/A"}</td>
                      <td>{Array.isArray(visit.service) ? visit.service.map((s) => (typeof s === "object" ? s.amount : s)).join(", ") : typeof visit.service === "object" ? visit.service.amount : visit.service || "N/A"}</td>
                      <td>
            ...
            <div className="modal-body">
              <form>
                <div className="mb-3">
                  ...
                  <input type="date" className="form-control" value={apptData.date} onChange={(e) => setApptData((prev) => ({ ...prev, date: e.target.value }))} />
                </div>
                <div className="mb-3">
                  ...
                  <div className="mb-3">
                    ...
                    <ul className="list-group mb-2">
                      {apptData.service.map((s, index) => (
                        <li key={s.id || s._id || index} className="list-group-item d-flex justify-content-between align-items-center">
                          <span>{s.name}</span>
                          <input
                            type="number"
                            ...
                            onChange={(e) => {
                              ...
                              setApptData((prev) => ({ ...prev, service: updatedServices, amount: total }));
                            }}
                          />
                        </li>
                      ))}
