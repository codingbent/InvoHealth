...
export default function UpdateAppointmentModal({
    ...
    const handleCheckboxChange = (serviceObj, checked) => {
        ...
        if (checked) {
            ...
            if (!exists) {
                updatedServices.push({
                    ...
                    id: serviceObj._id || null,
                    amount: serviceObj.amount || 0,
                });
    ...
    const handleUpdate = async () => {
        try {
            const res = await fetch(
                `/api/auth/updateappointment/${appointment.appointmentId}/${visit._id}`,
                {
                    method: "PUT",
                    headers: {
